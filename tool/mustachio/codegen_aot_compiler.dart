...
class _BlockCompiler {
  ...
  Future<void> _compilePartial(Partial node, Set<String> referenceUris) async {
    ...
    partialCompiler ??= _templateCompiler._partialCompilers
...
class _RenderersForPath {
  ...
  _RendererData? getSuitable(ContextStack contextStack) {
    for (var existingContextStack in _renderers.keys) {
      if (contextStack.length != existingContextStack.length) continue;
      var keyIsAllSubtypes = true;
      for (var i = 0; i < contextStack.length; i++) {
        if (!_typeSystem.isSubtypeOf(
            contextStack[i].type, existingContextStack[i].type)) {
  ...
  void put(ContextStack contextStack, _AotCompiler compiler, String renderer) {
    var existingContextStack = _existingKey(contextStack);
    if (existingContextStack == null) {
      _renderers[contextStack] = _RendererData(compiler, renderer);
      return;
    }
    var data = _renderers[existingContextStack]!;
    _renderers[_existingKey(contextStack)!] = _RendererData(compiler, renderer)
